...
- **⚠️ 设计语言切换中**：2026-08-29 项目所有者给定新设计语言——**微信 iOS 版（WeUI 风格）**：#EDEDED 底 + 白卡 + 0.5px 发丝线、品牌绿 #07C160 / 链接蓝 #576B95 / 警示红 #FA5151、系统字体（不引 web 字体）、扁平无阴影无渐变、按压态 #E5E5E5、iOS 右滑转场。**试点：首页 + 点菜页已切换**，其余 8 页仍为晨光厨房，待所有者确认气质后全量铺开。双人格语义保留：气泡绿/中性色区分 我/TA，功能逻辑零变更。
...
| WeUI 试点 | 新设计语言落地首页+点菜页：index.css 增 weui 令牌与组件类（.weui-theme/.weui-nav/.weui-tabbar/.weui-group/.weui-cell/.weui-search）、新增 WeuiTabbar（DockLayer 按路由切换）、Icons 增 cart/gear、两页重写为扁平白卡布局；晨光令牌与其余 8 页未动，待确认后全量 | 本轮 |
...
- **WeUI 全量铺开待所有者确认**：其余 8 页 + 后台仍为晨光风；iOS 右滑推入转场需改造 App.jsx 全局 pageEnter（现全局为纯 opacity 渐隐）；晨光 Hero/玻璃卡/弹簧动效在全量时退场。
- `preview-all-features.html` 同步基准仍是晨光版视觉，WeUI 全量后需再同步一次。
