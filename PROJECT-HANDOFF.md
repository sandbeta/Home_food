...
## 7.31 第七批 · 全菜单补做法（2026-09-26）

所有者要求「每一道菜都加上对应的做法」。盘点：`seedRecipes.js` 已有 342 条（HowToCook 灌库），菜单共 432 道，**缺 90 道**（老种子菜 1-10、菜系代表菜 11-65、夜宵小吃汤饮 900-924）。渲染链路（`DishDetail` 菜谱卡、`purchaseList` 原料归并、`avoid` 忌口扫描）本就现成，**纯数据缺口，代码零改动**。

- **补 90 条菜谱**：按菜系切 6 组并行撰写（A 川湘粤 / B 鲁苏浙 / C 闽徽东北 / D 西北云贵夜市 / E1 面点卤味 / E2 小吃懒人档 / F 汤饮家常菜），每条五字段 `ingredients/steps/difficulty/calories/tip`。技法写实（九转大肠先煮后炸再煨、起锅撒白糖；小笼汤包皮冻与馅 4:1；锅包肉必土豆淀粉复炸；热可可牛奶不可煮沸）；夜宵懒人档诚实使用半成品（冷冻手抓饼 / 泡面料包 / 袋装螺蛳粉），重点写「怎么把它弄到比摊上还好吃一点」。
- **守既有语料约定（本批最容易踩的坑）**：全库 94.8% 的 `ingredients` 是**裸原料名、不带数量单位**——因为 `purchaseList.normName` 按「剥括号后取第一个空格前」归并同名原料，写成「盐 适量」「五花肉 500g」会把同一食材拆成多条、把采购清单打散。数量一律写进 `steps`。另禁厨具进 ingredients（旧语料曾混入 36 种 53 次，靠 TOOL_RE 兜底过滤）。
- **合并校验器**（临时脚本，不入库）：复刻 `normName` 的真实归并逻辑作判据，查 id 完整性、五字段格式、`★` 档位、`N 大卡` 式样、tip 的 `- ` 前缀、原料含厨具/数量/空格截断，并拦住与既有 342 条的 id 冲突 → **90/90 实到、0 违规**才落盘。
- **双端同源**：`src/lib/seedRecipes.js`（前端动态 import）与 `server/data/seed-recipes.json`（家庭模式服务端）由同一份 JSON 序列化写出，**逐字节一致**。
- **门禁补强（本批真正的工程收益）**：`test_mockApi` 原有断言 `老种子菜(#1) recipe=null` 把「没菜谱」固化成了事实，补全后必然失败——改为断言新现实，并新增三条防回归锁：① 全菜单 432/432 都有菜谱 ② 432 条五字段格式全合规 ③ server 副本与前端逐字节一致。今后任何人删菜谱或双端漏同步，测试当场红。
- **注释诚实化**：3 处因本批失效的说明同步——`seedRecipes.js` 头（标注手工补 90 道及 id 区间）、`DishDetail` 菜谱卡（原「HowToCook 灌库菜才有」）、`purchaseList`（原「65 道老菜跳过」；noRecipe 分支保留给管理员新增菜与请求失败两种真实路径）。

**校验（四件套全绿 + dev 实测）**：lint 0 error（9 warning 全既有）/ 双测试全过（含 5 条菜谱断言）/ build ✅ / p6=0。dev 实测 `#/dish/1`：菜谱卡渲染、11 步、12 个原料丸（干净裸名）、难度 `★★★` 与 `880 大卡` 徽标可见、💡 贴士在位、console 零报错。新增 90 道共产生 275 个唯一原料名，无可疑名。

**payload 记账**：`seedRecipes` 懒加载 chunk 163KB→473KB raw（gzip 174KB）。它只在打开菜品详情时动态 import、不在首屏关键路径，且 sw.js 对静态资源 cache-first（看过一次即离线可用）——相对第五批省下的 7.7MB 属可接受量级。**若要再压**：按 id 区间拆多个 chunk，让打开一道菜只拉一道菜的量。
