    ...
    <div>
      <Header title="菜单管理" subtitle={`共 ${dishes.length} 道菜`}
        right={<motion.button whileTap={{ scale: 0.95 }} onClick={() => { setEditingDish(null); setShowModal(true) }}
          className="d3-btn d3-btn-primary px-4 py-2 rounded-xl text-xs font-bold text-[#2A1E0E]"
          >+ 添加</motion.button>} />
        ...
        {loading ? (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center py-20 relative overflow-hidden">
            ...
            <div className="absolute bottom-16 right-1/4 w-28 h-28 rounded-full opacity-10"
              style={{ background: 'radial-gradient(circle, rgba(194,199,210,0.35), transparent 70%)' }} />
          ...
          <div className="space-y-3">
            {dishes.map(dish => (
              <motion.div key={dish.id} layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div className="flex">
                  <div className="w-1 shrink-0" style={{ background: dish.available ? 'var(--color-gold)' : 'var(--color-mist)' }} />
                  <div className="p-3.5 flex-1">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1 mt-0.5">
                          ...
                          <span className="text-[14px] font-bold text-[var(--color-gold-soft)]">¥{dish.price}</span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 mt-2.5 pt-2.5 border-t border-[var(--color-glass-border)]">
                      <motion.button whileTap={{ scale: 0.95 }} onClick={() => handleToggle(dish)}
                        ...
                        style={dish.available ? { background: 'linear-gradient(135deg, #9DB39A, #7A9476)', boxShadow: '0 2px 8px rgba(157,179,154,0.25)' } : {}}>
                        {dish.available ? '✓ 上架' : '已下架'}
                      </motion.button>
                      <motion.button whileTap={{ scale: 0.95 }} onClick={() => { setEditingDish(dish); setShowModal(true) }}
                        className="flex-1 py-2 rounded-xl text-xs font-bold text-[#1A1D22]"
                        style={{ background: PERSONA.partner.gradient, boxShadow: '0 2px 8px rgba(194,199,210,0.2)' }}>编辑</motion.button>
                      <motion.button whileTap={{ scale: 0.95 }} onClick={() => handleDelete(dish.id)}
                        className="flex-1 py-2 rounded-xl text-xs font-bold text-white"
                        style={{ background: 'linear-gradient(135deg, #E8A6A0, #D7736B)', boxShadow: '0 2px 8px rgba(215,115,107,0.2)' }}>删除</motion.button>
                    </div>
                  </div>
                </div>
              </motion.div>
