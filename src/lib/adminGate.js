...
function askPassword() {
  ...
  gatePromise = new Promise((resolve) => {
    ...
    overlay.innerHTML = `
      <form class="cg-gate-card" style="background:var(--color-ink-900);border:1px solid var(--color-glass-border);border-radius:16px;padding:28px;width:min(88vw,320px);box-shadow:0 8px 32px rgba(43,38,32,.12)">
        <h2 style="font-size:17px;margin:0 0 6px;color:var(--color-bone)">🔐 管理验证</h2>
        ...
        <input name="password" type="password" placeholder="家庭管理密码" autofocus
          style="width:100%;box-sizing:border-box;padding:10px 12px;border:1px solid var(--color-glass-border);border-radius:10px;font-size:15px;background:transparent;color:var(--color-bone)">
        <div class="cg-gate-err" style="min-height:18px;font-size:12px;color:var(--color-clay);margin-top:8px"></div>
        <button type="submit" style="margin-top:6px;width:100%;padding:10px;border:0;border-radius:10px;background:var(--color-clay);color:var(--color-ink-900);font-size:15px;font-weight:700">验证并继续</button>
        <button type="button" name="cancel" style="margin-top:10px;width:100%;padding:8px;border:0;border-radius:10px;background:transparent;color:var(--color-ash);font-size:13px">取消</button>
      </form>`
