...
首页第一焦点（夜宵页为宵夜变体）：机顶羊毛檐 + 机名 + `No.xx` 糖牌 + **泡泡时钟**（clay 实底胶囊，实时时间，全站唯一的时间显示），机腹玻璃罩内吊着官方角色素材与主推菜圆盘，底部出菜口面板（菜名 + caramel 价格 + 换一道）。**「换一道」= 提走旧的 → 放下新的**（≈0.95s，低频装饰动效，不受 UI 300ms 档约束），自动轮换时底部走一条 clay 进度条预告。

⚠️ 工程纪律：**不要给这台机器画爪钩/横梁/缆线**。官方角色素材（`public/lazy-assets/`）本身就是完整的抓娃娃场景——画面内已含横梁 + 缆线 + 带爱心的爪钩 + 被夹住的角色；再叠一层自绘机构会重影。素材用固定 150×150 盒 + `object-contain` + 顶对齐（池内四张纵横比都 <1，contain 后高度撑满 → 素材自带的横梁天然贴盒顶，四个挂点绝对对齐）。

### 角色素材：官方图 × 自绘小羊的分工
- **官方素材**（`theme/characters.js` → `ui/Character.jsx`）：大尺寸与静态肖像位——页头徽记（白天懒羊羊 / 夜宵灰太狼值班）、空态主图、娃娃机吊挂位。素材自带配色，不继承 `currentColor`，**必须给托底容器**（圆底或卡片）。
...
- **Don't** 给娃娃机再画爪钩/横梁（官方素材自带，会重影）；Don't 把官方角色素材放进小于 30px 的槽位（会读不出是谁，用自绘 LazySheep）
